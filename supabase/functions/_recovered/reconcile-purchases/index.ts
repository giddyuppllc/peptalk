...
//
// `reconcile-purchases` (deployed version 1) is live on the PepTalk Supabase project and has
...
// `GET /v1/projects/{ref}/functions/reconcile-purchases/body`, which returns an ESZIP
...
// `supabase/functions/reconcile-purchases/index.ts`, and only then deploy.
...
/** Mirrors PRODUCT_TO_TIER in validate-purchase. */ const PRODUCT_TO_TIER = {
...
/** Service-account access token for the Play Developer API. */ async function googleToken() {
 ...
 */ async function recheckGoogle(productId, token, access) {
  const url = `https://androidpublisher.googleapis.com/androidpublisher/v3/applications/` + `${ANDROID_PACKAGE_NAME}/purchases/subscriptions/${productId}/tokens/${encodeURIComponent(token)}`;
...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    const { data: rows, error } = await admin.from('purchase_validation_log').select('id, user_id, platform, product_id, purchase_token, external_id, stage, created_at').in('stage', [
      'verified',
      'grant_failed'
    ]).order('created_at', {
      ascending: true
    ...
    if (error) {
      ...
      return json({
        error: 'Could not read the validation log'
      }, 500);
    }
    ...
    for (const row of rows ?? []){
      ...
      try {
        ...
        const { data: existing } = await admin.from('subscriptions').select('id, is_active, expires_at').eq('user_id', row.user_id).eq('product_id', row.product_id).maybeSingle();
        if (existing?.is_active) {
          if (!dryRun) {
            await admin.from('purchase_validation_log').update({
              stage: 'granted',
