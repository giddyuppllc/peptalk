...
//
// `credit-autorefill` (deployed version 1) is live on the PepTalk Supabase project and has
...
// `GET /v1/projects/{ref}/functions/credit-autorefill/body`, which returns an ESZIP
...
// `supabase/functions/credit-autorefill/index.ts`, and only then deploy.
...
const SQUARE_BASE = SQUARE_ENV === 'production' ? 'https://connect.squareup.com' : 'https://connect.squareupsandbox.com';
/** The one pack. Auto-refill buys exactly this and nothing else. */ const REFILL_SKU = 'peptalk_credits';
...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    if (!pack) return json({
      error: 'Refill SKU not in catalog'
    }, 500);
    if (!SQUARE_ACCESS_TOKEN || !SQUARE_LOCATION_ID) {
      return json({
        skipped: 'square_not_configured'
      });
    }
    // Is the balance actually low? Checked here rather than trusting the
    // caller, so a buggy caller cannot cause a charge.
    const { data: bal } = await admin.from('ai_credit_balance').select('balance_microcents').eq('user_id', userId).maybeSingle();
    const { data: cfg } = await admin.from('ai_credit_autorefill').select('enabled, threshold_microcents').eq('user_id', userId).maybeSingle();
    if (!cfg?.enabled) return json({
      skipped: 'not_enabled'
    });
    ...
    try {
      ...
      const card = (cards.data?.cards ?? []).find((c)=>c?.enabled !== false);
      if (!card?.id) {
        await admin.rpc('record_autorefill_result', {
          p_user_id: userId,
          p_success: false,
          p_error: 'no_card_on_file'
        });
        return json({
          skipped: 'no_card'
        });
      ...
      if (!pay.ok || status !== 'COMPLETED' || !paymentId) {
        const detail = String(pay.data?.errors?.[0]?.detail ?? pay.data?.errors?.[0]?.code ?? `status_${status}`);
        await admin.rpc('record_autorefill_result', {
          p_user_id: userId,
          p_success: false,
          p_error: detail
      ...
      if (grantErr) {
        ...
        console.error(`[credit-autorefill] CHARGED ${paymentId} BUT GRANT FAILED for ${userId}:`, grantErr);
      ...
      await admin.rpc('record_autorefill_result', {
        p_user_id: userId,
        p_success: false,
        p_error: String(inner).slice(0, 200)
      });
      throw inner;
    }
  } catch (err) {
